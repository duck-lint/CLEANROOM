...
This report contains evidence from two distinct times:

- **Original pre-archive PR #23 run:** the retained private
  `vault-observation/v3` artifact was available, the corrected Phase 5 v2
  projection was constructed twice, and the historical hashes and census
  below were recorded.
- **2026-08-22 recovery:** the retained private observation artifact was not
  available locally. This recovery mechanically restored the source, schema,
  test, and documentation representation and validated those public surfaces;
  it did not access the authored vault or observer repository, reconstruct the
  private projection, or independently reproduce the historical hashes or
  corpus results below.

The historical hashes, identities, and corpus census are preserved as
historical evidence. They are not claims about output regenerated by the
2026-08-22 recovery.
...
In the original pre-archive PR #23 run, the constructor hashed the exact input
bytes before parsing, then required the v3 schema, observer commit, and
specimen identity above. Both historical construction runs consumed that exact
retained artifact. The authored vault and observer repository were not accessed
or mutated, and no live-vault hydration occurred.

## Corrected projection identity from the original run
...
## Historical corpus and projection census
...
In the original pre-archive PR #23 run, the exact pinned observation was
constructed twice into separate private output files:
